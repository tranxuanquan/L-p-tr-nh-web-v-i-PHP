...
if (isset($_POST['qty']) && isset($_POST['id']) && !isset($_POST['add_to_cart']) && !isset($_POST['remove_cart'])) {
    $id = $_POST['id'];
  ...
  <section class="cart-section">
    <div class="container">
      ...
      <div class="cart-summary">
        ...
        <button type="button" class="btn" style="font-size:1.1rem;" onclick="openModal()">Đặt hàng</button>
      </div>
    </div>
  </section>

  <!-- Thanh toán popup -->
<div id="checkout-modal" style="display:none;position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,.25);z-index:999;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:14px;max-width:350px;width:90%;padding:28px 22px;box-shadow:0 8px 32px rgba(0,0,0,.18);position:relative;">
    <button onclick="closeModal()" style="position:absolute;top:12px;right:16px;background:none;border:none;font-size:22px;color:#b48d61;cursor:pointer;">&times;</button>
    <h3 style="margin-top:0;color:#b48d61;">Chọn phương thức thanh toán</h3>
    <form method="post" action="order.php">
      <div style="margin-bottom:18px;">
        <label><input type="radio" name="payment" value="cash" checked> Tiền mặt khi nhận hàng</label><br>
        <label><input type="radio" name="payment" value="qr"> Thanh toán qua QR</label>
      </div>
    ...
    <div id="qr-section" style="display:none;margin-top:18px;text-align:center;">
      <img src="images/qr.png" alt="QR Code" style="width:160px;">
      <div style="margin-top:8px;color:#5b4632;">Quét mã để thanh toán</div>
    </div>
  </div>
</div>
...
// Hiển thị/hide phần QR code khi chọn phương thức thanh toán
